...
f = open(path + 'shawshank_scene_facesize.csv', 'r')
reader = csv.reader(f)
lines = []
for line in reader:
	lines.append(line)
scene_start = [int(line[9]) for line in lines[1:158]] 
skeet_sizes = [[0.0, 0.0] for i in range(len(scene_start))]

for row_line in lines[1:]:
	try:
		frame = int(row_line[25][11:16])
...
for start, mean in zip(scene_start, skeet_sizes_mean):
	print('start scene: {}, skeet mean face size for scene: {}'.format(start, mean))
